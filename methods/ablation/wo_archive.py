...
def advanced_nsga2_wo_archive(p_times, pop_size, n_gen, seed):
    random.seed(seed)
    np.random.seed(seed)
    ...
    pbar = tqdm(total=n_gen, desc="w/o Archive")
    ...
    pareto_fitness = np.array([s.fitness for s in archive]) if archive else np.array([])
