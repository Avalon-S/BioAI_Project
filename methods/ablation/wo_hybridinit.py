...
def advanced_nsga2_wo_hybridinit(p_times, pop_size, n_gen, seed):
    random.seed(seed)
    np.random.seed(seed)
    ...
    # 去除混合初始化，仅使用 standard_generator
    population = [standard_generator(rnd, args) for _ in range(pop_size)]

    fitness = standard_evaluator(population, args)
    archive = []
    pbar = tqdm(total=n_gen, desc="w/o HybridInit")
        ...
        for front in fronts:
            ...
            if selected_count + front_size <= pop_size:
                ...
                if gen_ratio < 0.3:
                    ...
                    diversity_weight = 0.8 * (1 - gen_ratio)**2
                    convergence_weight = 0.2 * gen_ratio**0.5
